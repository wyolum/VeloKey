...
class ClickButton
{
  public:
    ...
    boolean depressed;            // the currently debounced button 
                                  // (press) state (presumably it is not sad :)
    long debounceTime;
    long multiclickTime;
    long longClickTime;
  private:
    ...
    int _clickCount;              // Number of button clicks within 
                                  // multiclickTime milliseconds
    long _lastBounceTime;         // the last time the button input pin was
                                  // toggled, due to noise or a press
};
